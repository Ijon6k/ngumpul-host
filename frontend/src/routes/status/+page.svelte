	...
	/>
		...
		{#if $statusQuery.isPending && !$statusQuery.data}
			<div class="flex flex-col gap-6" role="status" aria-live="polite">
				<div class="h-32 bg-(--bg-surface) border border-(--border-hairline) rounded-xl p-6 flex flex-col gap-4" aria-hidden="true">
					<Skeleton variant="line" class="w-40" />
					<Skeleton variant="line" class="w-24" />
					<Skeleton variant="line" class="w-1/2" />
				</div>
				<div class="h-36 bg-(--bg-surface) border border-(--border-hairline) rounded-xl p-6 flex flex-col gap-4" aria-hidden="true">
					<Skeleton variant="line" class="w-1/3" />
					<Skeleton variant="block" class="!h-20" />
				...
				<div class="h-44 bg-(--bg-surface) border border-(--border-hairline) rounded-xl p-6 flex flex-col gap-4" aria-hidden="true">
					<Skeleton variant="line" class="w-1/3" />
					{#each Array(3) as _}
