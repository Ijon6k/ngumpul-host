...
func (s *Service) CalculateAvailability(ctx context.Context, rangeFilter string) (*AvailabilityResponse, error) {
	...
	switch rangeFilter {
	case "1d":
		// 24 hours * 2 intervals/hour = 48 30-minute blocks (high-resolution sweet spot)
		windowStart = now.Add(-24 * time.Hour)
		blockCount = 48
		blockDuration = 30 * time.Minute
	case "7d":
		// 7 days * 8 intervals/day = 56 3-hour blocks (high-resolution sweet spot)
		windowStart = now.Add(-7 * 24 * time.Hour)
		blockCount = 56
		blockDuration = 3 * time.Hour
	default: // 30d
	...
	// Calculate discrete observation blocks
	...
	for i := blockCount - 1; i >= 0; i-- {
		...
		// Check if this block is completely prior to the monitoring baseline
		if bEnd.Before(firstMonitoredAt) {
