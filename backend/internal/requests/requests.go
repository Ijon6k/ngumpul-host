...
func (h *Handler) Submit(w http.ResponseWriter, r *http.Request) {
	...
	if req.ProjectName == "" || req.RepositoryURL == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "Project name and repository URL are required"})
		return
	}
	...
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "Failed to submit hosting request"})
		return
	}

	// Notify admins (insert notification)
...
func (h *Handler) ListMyRequests(w http.ResponseWriter, r *http.Request) {
	...
	list := make([]HostingRequest, 0)
	for rows.Next() {
		var hr HostingRequest
		if err := rows.Scan(
...
func (h *Handler) AdminApprove(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]string{"message": "Request approved"})
}
...
func (h *Handler) AdminComplete(w http.ResponseWriter, r *http.Request) {
	...
	if req.PublicURL == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "Public URL is required to complete hosting setup"})
		return
	}
	...
	slug := projects.GenerateSlug(hr.ProjectName)
	...
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "Failed to register project: " + err.Error()})
		return
	}
